...
std::string serialaze(const std::vector<bid> &bids) {
  ...
  for (bid b : bids) {
    res += std::to_string(b.bid_id) +
           ": {\"price\": " + std::to_string(b.price) + ", \"owner\": \"" +
           b.owner + "\", \"active\": {";
    ...
    res += "}, \"amount\": " + std::to_string(b.amount) +
           ", \"resolved\": " + std::to_string(b.resolved) + "},";
  }
  res.pop_back();
  res += "}}";
  return res;
...
LinkedListNode::LinkedListNode(std::string x,
                               std::shared_ptr<LinkedListNode> n) {
  val = 0;
  parent = n;
  stringVal = x;
  number = 0;
...
LinkedListNode::LinkedListNode(actives::deals::bid x,
                               std::shared_ptr<LinkedListNode> n) {
  val = 0;
  parent = n;
  bidVal = x;
...
void LinkedList::push(int value) {
  ...
  cv.wait(lock, [this] { return !busy; });
  busy = true;
  std::shared_ptr<LinkedListNode> newNode =
      std::make_shared<LinkedListNode>(value);
  if (this->first == nullptr) {
    this->first = newNode;
    this->last = newNode;
    return;
...
bool LinkedList::empty() { return this->first == nullptr; }
} // namespace utils
