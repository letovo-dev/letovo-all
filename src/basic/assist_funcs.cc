...
parse_page_content(rapidjson::Document &new_body) {
  ...
  // if(new_body.HasMember("is_published") &&
  // new_body["is_published"].GetBool()) {
...
void add_id_to_page(
  ...
  if (!std::filesystem::exists(filePath)) {
    logger_ptr->error([filePath] {
      return fmt::format("File does not exist: {}", filePath);
    });
    return;
  }
  ...
  if (!inFile) {
    logger_ptr->error([filePath] {
      return fmt::format("Failed to open file for reading: {}", filePath);
    });
    return;
  }
  ...
  if (!outFile) {
    logger_ptr->error([filePath] {
      return fmt::format("Failed to open file for writing: {}", filePath);
    });
    return;
  }
  ...
  logger_ptr->info(
      [filePath] { return fmt::format("Added id to file: {}", filePath); });
}
...
void create_file(
  ...
  logger_ptr->info(
      [filePath] { return fmt::format("Created file: {}", filePath); });
}
...
void create_mdx_from_template_file(
  ...
  std::string mdx_template_text =
      ...
      "fetchPokemon();\n  }, []);\n  if (!pokemon) {\n    return "
      "<h1>Загрузка...</h1>;\n  }\n  return "
      "<pre>{JSON.stringify(pokemon[\"result\"][0][\"text\"], null, "
      "2).slice(1, -1)}</pre>;\n};\n\n<PokemonData />\n";

  create_file(Config::giveMe().pages_config.wiki_path.absolute + filePath,
              mdx_template_text, logger_ptr);
}

void create_mdx_from_template_file(
    const std::string &filePath, const std::string &title,
    const std::string &author_id, const std::string &post_id,
    std::shared_ptr<restinio::shared_ostream_logger_t> logger_ptr) {
  std::string mdx_template_text =
      "import { useState, useEffect } from \"react\";\nimport { AuthorCard } "
      "from '../src/author_card';\n\n# " +
      title +
      "\n\nexport const AuthorCardElement = () => {\n  const [pokemon, "
      "setPokemon] = useState(null);\n  useEffect(() => {\n    const "
      ...
      "fetchPokemon();\n  }, []);\n  if (!pokemon) {\n    return "
      "<h1>Загрузка...</h1>;\n  }\n    return <AuthorCard \n    "
      "name={JSON.stringify(pokemon[\"result\"][0][\"username\"]).slice(1, "
      "-1)}\n   "
      "ava={JSON.stringify(pokemon[\"result\"][0][\"avatar_pic\"]).slice(1, "
      "-1)}\n    authorLink={\"/user/page/\" + "
      "JSON.stringify(pokemon[\"result\"][0][\"username\"]).slice(1, -1)} \n   "
      " />\n};\n\nexport const PokemonData = () => {\n  const [pokemon, "
      "setPokemon] = useState(null);\n  useEffect(() => {\n    const "
      "fetchPokemon = () => {\n        fetch('/api/post/" +
      post_id +
