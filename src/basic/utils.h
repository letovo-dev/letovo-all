...
struct bid {
  ...
  bid(int p, int b_id, const std::string &o, actives::active_obj act, int am,
      bool r = false)
      : price(p), bid_id(b_id), owner(o), active(act), amount(am), resolved(r) {
  }
...
public:
  ...
  LinkedListNode(actives::deals::bid,
                 std::shared_ptr<LinkedListNode> n = nullptr);
