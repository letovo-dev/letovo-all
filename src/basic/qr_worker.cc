...
void achivement_qr(
  ...
  router.get()->http_get(
      R"(/achivements/qr/:achivement_id([0-9]+))",
      [pool_ptr, logger_ptr](auto req, auto params) {
        ...
        std::string data = fmt::format("https://{}/ach_getter/{}/{}",
                                       Config::giveMe().server_config.adress,
                                       achivement_id, username);
        std::string filename_str =
            fmt::format("achivmenet_{}_{}.png", achivement_id, username);
...
void page_qr(std::unique_ptr<restinio::router::express_router_t<>> &router,
             std::shared_ptr<cp::ConnectionsManager> pool_ptr,
             std::shared_ptr<restinio::shared_ostream_logger_t> logger_ptr) {
  router.get()->http_get(R"(/post/qr/:post_id([0-9]+))", [pool_ptr, logger_ptr](
                                                             auto req,
                                                             auto params) {
