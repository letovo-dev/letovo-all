...
string hash_from_string(const string &input) {
  ...
  if (Config::giveMe().server_config.update_hashes) {
    str_to_hash =
        input + to_string(chrono::duration_cast<chrono::seconds>(
                              chrono::system_clock::now().time_since_epoch())
                              .count());
  } else {
    str_to_hash = input;
  }
  SHA256(reinterpret_cast<const unsigned char *>(str_to_hash.c_str()),
         input.size(), hash);
...
std::string change_username(std::string old_username,
