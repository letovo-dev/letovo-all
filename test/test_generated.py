...
def test_get_news():
    start = 0
    size = 10
    response = requests.get(f'{URL}/social/news/search?start={start}&size={size}', headers={'Bearer': TOKEN}, verify=False)
    assert response.status_code == 200
...
def test_get_comments():
    ...
    start = 0
    size = 10
    response = requests.get(f'{URL}/social/comments/search?post_id={post_id}&start={start}&size={size}', headers={'Bearer': TOKEN}, verify=False)
    assert response.status_code == 200
    data = response.json()
    assert 'result' in data
...
def test_achivements_tree():
    ...
    for achievement in result:
        ...
        assert 'achivement_decsription' in achievement, "Achievement dictionary does not contain 'achivement_decsription'"
