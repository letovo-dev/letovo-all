...
- [ ] Implement `mac-supervisor.sh` as the forced command: accept only `run`, validate/extract the archive, acquire non-blocking `flock`, emit the start marker, create a run-scoped Lima clone, copy the source/control files, run `timeout --kill-after=30 1800`, return the result archive on stdout, and trap cleanup of only its process group/VM/state directory.
