...
def add_logs_to_file(file_path: str):
    ...
    with open(file_path, "w") as file:
        for line in lines:
            ...
            elif FUNCTION_LINE.search(
                line
            ):  # and FUNCTION_LINE.search(line).group(1) in used_types and "server" in last_namespace:
                global last_function
                # last_function =
                file.write(
                    "            logger_ptr->trace([]"
                    + "{"
                    + f'return "called {FUNCTION_LINE.search(line).group(1)}";'
                    + "}"
                    + ");\n"
                )
